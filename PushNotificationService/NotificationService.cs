...
namespace PushNotificationService
{
    ...
    public class NotificationService
    {
        ...
        public async Task SendAll(string _title, string _msg, string _image)
        {
            foreach (var token in m_Tokens)
            {
                await SendUser(token.Key, _title, _msg, _image);
            }
        }
        ...
        public async Task SendUser(int _id, string _title, string _msg, string _image)
        {
            if (!m_Tokens.ContainsKey(_id))
            {
            ...
            {
                NotificationResult result = await SendMessage(token, client, _title, _msg, _image);
                ...
                var payload = new
                {
                    message = new
                    {
                        ...
                        data = new
                        {
                            title = _title,
                            body = _msg,
                            image = $"https://pushnotificationbackend-kvi4.onrender.com/images/{_image}"
                        }
                    }
                };
